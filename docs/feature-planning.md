...
2. **Archive trigger — on bag open:**
    - When a user opens a new bag, check if any existing _active_ (non-archived) bags share the
      same `name + roasterName`.
    - If so, prompt the user: _"You already have '{name}' open. Archive the previous bag and its
      brews?"_
...
3. **Timeline filtering:**
    - The main timeline should default to showing only _active_ (non-archived) bags and their
      brews.
